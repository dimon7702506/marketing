...
if (isset($_POST['save']) || isset($_POST['copy'])){
    ...
    if (empty($errors)){
        ...
        header("location: ./element_ukr.php?id=$id");
    }
}
...
if (isset($_POST['close'])) {
    ...
    if (isset($_COOKIE['text_search']) && (isset($_COOKIE['field_search']))){
        $str_search = './names_ukr.php?search='. $_COOKIE['text_search'] . '&field_search=' . $_COOKIE['field_search'] . '&submit_search=search';
    }else{
        $str_search = './names_ukr.php';
    }
    header("location: $str_search");
}

require_once "./element_ukr.html";