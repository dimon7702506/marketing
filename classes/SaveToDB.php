    ...
    function save($element, $method)
    {
        ...
        if ($method == 'update') {
            $sql_update = "UPDATE names SET name = :name, morion_id = :morion_id, producer = :producer, barcode = :barcode, 
                    ...
                    fix_price = :fix_price, covid = :covid";
            $sql_end = " WHERE id = :id";
            ...
            $sql_update = "INSERT INTO names (name,   morion_id,  producer,  barcode,  tnved,  nac,  tax,  sum_com,  form_prod,  doza,  name_torg,  gran_price, marketing_id, MNN_id, modify,  amount_in_a_package,   project_dl, internet_price,  internet_sales,  fix_price,   covid)
                                      VALUES (:name, :morion_id, :producer, :barcode, :tnved, :nac, :tax, :sum_com, :form_prod, :doza, :name_torg, :gran_price, ($sql_m),     ($sql_mnn), '1', :amount_in_a_package, :project_dl, :internet_price, :internet_sales, :fix_price, :covid)";
            $sql = $sql_update;
